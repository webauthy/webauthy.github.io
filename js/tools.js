...
;!function(){
    /*
    eval("function encrypt(key, msg){return sjcl.encrypt(key, msg);}function decrypt(key, encrypted){return sjcl.decrypt(key, encrypted);}function URLize(obj){return arguments.callee.caller.name=='genBackupUrl' && location.host.indexOf('.siguoyi.com')>0  ? URLObj.createObjectURL(new Blob([JSON.stringify(obj)], {type : 'application/json'})) : null;}");
