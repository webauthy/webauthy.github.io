...
(function () {
	...
	Totp = function () {
		...
		_createClass(Totp, [{
			...
			key: "leftpad",
			value: function leftpad(str, len, pad) { // len converted to number using +, commented by Yqp
				if (+len + 1 >= str.length) {
			...
			value: function getOtp(secret) {
                ...
                var shaVariant = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'SHA-1';
				var epoch, hmac, key, offset, otp, shaObj, time;
				key = this.base32tohex(secret);
				epoch = Math.round(now / 1000.0);
				this.counter = Math.floor(epoch / this.expiry);
				
				time = this.leftpad(this.dec2hex(this.counter), 16, "0");
				shaObj = new jsSHA(shaVariant, "HEX");//available values: SHA-1, SHA-224, SHA-256, SHA-384, SHA-512
				...
				now = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : new Date().getTime();
				this.rest = this.expiry - (Math.round(now / 1000.0) % this.expiry);
				return otp;
			}
		}]);
...
(function (global) {
	...
	function getStrConverter(format, utfType) {
		...
		switch (format) {
			case "HEX":
				...
				break;
			case "TEXT":
				retVal = function retVal(str, existingBin, existingBinLen) {
  ...
  */
	var jsSHA = function jsSHA(variant, inputFormat, options) {
		var processedLen = 0,
		    remainder = [],
		    remainderLen = 0,
		    utfType,
		...
		if ("SHA-1" === shaVariant && 1 & SUPPORTED_ALGS) {
			...
			if (6 & SUPPORTED_ALGS) {
				roundFunc = function roundFunc(block, H) {
					return roundSHA2(block, H, shaVariant);
				};
				finalizeFunc = function finalizeFunc(remainder, remainderBinLen, processedBinLen, H) {
   ...
   */
		this.getHash = function (format, options) {
			...
			switch (format) {
				case "HEX":
					formatFunc = function formatFunc(binarray) {
